...
export default function BottomMenuBar(userInformationList: any, currentDirection: string, photo:any, name:any) {
  ...
  useEffect(() => {
    switch (value) {
      ...
      case "report":
        navigate('/Report', { state: userInformationList });
        break;
  ...
  return <div>
    ...
    <BottomNavigation value={value} onChange={handleChange} sx={{ backgroundColor: 'transparent', boxShadow: 'none', width: 1100 }}>
      <BottomNavigationAction label="Main Page" value="home" icon={<HomeIcon />} sx={{ '&.Mui-selected': { color: 'rgba(245, 245, 245, 0.9)' } }} />
      ...
      <BottomNavigationAction label="Report" value="report" icon={<AssignmentIcon />} sx={{ '&.Mui-selected': { color: 'rgba(245, 245, 245, 0.9)' } }} />
      <BottomNavigationAction label="Profile" value="profile" icon={<Avatar alt={name} src={photo} />} sx={{ '&.Mui-selected': { color: 'rgba(245, 245, 245, 0.9)' } }} />
    </BottomNavigation>
  </div>
}
